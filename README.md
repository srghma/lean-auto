...
You should phrase this a bit more explicitly: the failure is not “gcc missing”, it’s specifically the linker `ld` missing from the environment. `gcc` invokes `collect2`, which then tries to call `ld`.
